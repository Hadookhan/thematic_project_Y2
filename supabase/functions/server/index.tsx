...
// Get all actors/directors
app.get("/make-server-e82f3bbc/actors", async (c) => {
  try {
    const actors = await kv.getByPrefix("actor:");
    return c.json({ actors: actors || [] });
  } catch (error) {
    console.log("Error fetching actors:", error);
    return c.json({ error: "Failed to fetch actors", details: String(error) }, 500);
  }
});

// Add new actor/director
app.post("/make-server-e82f3bbc/actors", async (c) => {
  try {
    const actor = await c.req.json();
    const id = actor.id || crypto.randomUUID();
    await kv.set(`actor:${id}`, { ...actor, id });
    return c.json({ success: true, actor: { ...actor, id } });
  } catch (error) {
    console.log("Error adding actor:", error);
    return c.json({ error: "Failed to add actor", details: String(error) }, 500);
  }
});

// Seed actors (for initial data load)
app.post("/make-server-e82f3bbc/actors/seed", async (c) => {
  try {
    const { actors } = await c.req.json();
    const keys = actors.map((actor: any) => `actor:${actor.id}`);
    await kv.mset(keys, actors);
    return c.json({ success: true, count: actors.length });
  } catch (error) {
    console.log("Error seeding actors:", error);
    return c.json({ error: "Failed to seed actors", details: String(error) }, 500);
  }
});
